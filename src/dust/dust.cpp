...
DustGasDrag::DustGasDrag(MeshBlockPack *ppack, ParameterInput *pin) :
    ...
    pmy_pack(ppack),
    particle_cache_i("dust_particle_cache_i",npm_cache_index,1),
    particle_cache_r("dust_particle_cache_r",npm_cache_real,1) {
  ...
  if (back_reaction) {
    int npart_capacity = std::max(1, static_cast<int>(ppar->prtcl_rdata.extent(1)));
    Kokkos::realloc(particle_cache_i, npm_cache_index, npart_capacity);
    Kokkos::realloc(particle_cache_r, npm_cache_real, npart_capacity);
  }
